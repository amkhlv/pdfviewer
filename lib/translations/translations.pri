defineReplace(tsFilesInDir) { # this macro returns a list containing the .ts files with their path (as specified in the first argument)
	...
	for(lang, languages) {
		tsName = $${dir}/pdfviewlib_$${lang}.ts # this assumes that names the .ts files are of the form pdfviewlib_fr.ts
#		exists($$tsName) {
			tsfiles += $$tsName
#		}
	}
...
!isEmpty(TRANSLATIONS) {
	...
	translations.path = $$PDFVIEWLIB_TRANSLATIONS_INSTALL_DIR
